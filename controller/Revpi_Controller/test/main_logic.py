...
from test_cases.logic_gate_case import run_logic_case

logger = logging.getLogger("main_logic")

# ============================== # ASYNC MAIN RUNTIME # ==============================
async def async_main(host: str, port: int, controller_id: str):
    init_db()
    init_csv()
    logger.info("Logic case logger initialized")

    result = run_logic_case("XOR", True, False)
    logger.info("Logic XOR result: %s", result)

    while True:
        try:
            await run_client(host, port, controller_id)
        except Exception as e:
            logger.exception("Logic case disconnected: %s", e)
